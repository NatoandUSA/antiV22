/* 22Etsy Pattern Evidence Harvester v3.6.0 - background service worker.
 *
 * Single job: relay a captured-evidence payload to the 22etsy agent's
 * /api/import endpoint. Read-only import relay; no marketplace writes, no image fetch.
 */
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  fetch(url, {
    ...
    sendResponse({
      ok: false,
      status: 0,
      error: error.name === "AbortError" ? "Request timed out after 30 seconds. Try chunking/less rows or check /api/import." : error.message
    });
  }).finally(() => {
