...
score for the Publish Gate and the discovery pages. Replaces the old hardcoded
can-we-win optimism (every niche ~70-80) with a weighted composite:
...
CONFIG = Path("config/scoring_weights.json")
...
def _private(keyword):
    """Private boost from our own proven winners (learning.py). None when the shop
    has NO history yet (normal pre-launch) so it neither helps nor hurts."""
    if not keyword:
        return None
    try:
        from src import learning
        wk = {str(k).lower() for k in learning.winner_keywords()}
        wt = {str(t).lower() for t in learning.winner_tags()}
    except Exception:  # noqa: BLE001
        return None
    if not wk and not wt:
        return None                      # no history at all -> neutral/omit
    kw = keyword.lower()
    toks = set(kw.split())
    winner_words = {w for t in wt for w in t.split()}
    if kw in wk or (toks & winner_words):
        return 80.0                      # matches a proven winner
...
def _feasibility(keyword, mode):
    ...
    try:
        ...
        pass
    ip_pen = {"low": 0, "medium": -12, "high": -30}[ip_risk]
